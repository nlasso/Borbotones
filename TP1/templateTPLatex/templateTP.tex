...
\asegura{(\forall a \in result) noGanoMedallas(j,a)}
\asegura{(\forall x \in result,y \in atletasSinMedallasPorPais(j,p)) competenciasDelAtleta(j,x) > competenciasDelAtleta(j,y)}
...
\aux{mejoresAtletas}{j:JJOO}{[Atleta]}{ [ ranking(c)_0 | c \in competenciasFinalizadas(j) ] }
\aux{paisesConRepetidos}{j: JJOO}{[Pais]}{ [ nacionalidad(a) | a \in atletas(j) ] }

\aux{orosPorPais}{l:[Atleta]}{\ent}{ [ l |  ] }
\end{problema}
