...
\aux{atletasYCompetencias}{j:JJOO, p: Pa\'{i}s}{[(Atleta,\ent)]}{[(x, y)|x \leftarrow atletasDelPais(j,p),y = long([z_i | h \leftarrow [1..cantDias(j)], z \leftarrow cronograma(j,h), i\leftarrow [0..|z|-1],j \leftarrow [4..|z|-1], alguno(x == participantes(z_i)) \wedge alguno(ranking(z_i)_j == x)])]}
