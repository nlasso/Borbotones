...
\asegura{(\forall c \in cronograma( pre(j),jornadaActual(pre(j))) |lesTocaControlAntiDoping(c)|==1 )}
\asegura{|atletasLesTocoAntiDoping(j,jornadaActual(pre(j)))|}
\asegura{(\exists a \in atletasLesTocoAntiDoping(j,jornadaActual(pre(j)))) |atletasLesDioPositivo(j,jornadaActual(pre(j))) 0 \leq |*100/|atletasLesTocoAntiDoping(j,jornadaActual(pre(j)))| \leq 5 )}
